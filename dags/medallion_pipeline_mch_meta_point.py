...
ICEBERG_NS = "mch"
BRONZE_TABLE = f"{ICEBERG_NS}.bronze_customers"
SILVER_TABLE = f"{ICEBERG_NS}.silver_customers"
GOLD_TABLE = f"{ICEBERG_NS}.gold_customer_summary"
...
@dag(
    dag_id="medallion_pipeline_mch_meta_point",
    ...
    params={
        ...
        "file": Param(default="ogd-local-forecasting_meta_point.csv", type="string"),
    },
)
def sample_data_pipeline(**_):
    ...
    def bronze_ingest(params: dict[str, Any], **_):
        ...
        FILE = params.get("file", "ogd-local-forecasting_meta_point.csv")
        RAW_KEY = f"{ORG}/{DATASET}/{FILE}"
        TABLE = f"{DATASET}_meta_point"
    ...
    def silver_clean(**_):
        ...
        DATASET = "point-forecast"
        BRONZE_TABLE = f"{ICEBERG_NS}.{DATASET}_meta_point"
        SILVER_TABLE = f"{ICEBERG_NS}.{DATASET}_meta_point_cleaned"
        ...
        silver = geopandas.GeoDataFrame(
            ...
            geometry=geopandas.points_from_xy(
                df["point_coordinates_lv95_east"], df["point_coordinates_lv95_north"]
            ),
            crs="EPSG:2056",
        )
        ...
        arrow_table = pa.table(silver.to_arrow())
    ...
    def gold_aggregate(**_):
        ...
        cat = _catalog()
        silver = cat.load_table(SILVER_TABLE)
